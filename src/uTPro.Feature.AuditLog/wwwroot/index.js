...
export class UtproAuditLogDashboard extends UmbLitElement {
    ...
    async _loadFilters() {
        try { const [t,h] = await Promise.all([this._api(API+'/event-types'),this._api(API+'/log-headers')]);
            this._eventTypes=t||[]; this._logHeaders=h||[]; } catch(e) { console.error(e); }
    }
    async _load() {
        this._loading=true;
        try { const ep = this._activeTab==='audit'?'audit-entries':'log-entries';
    ...
    _switchTab(t) { this._activeTab=t; this._skip=0; this._filterEventType=''; this._filterSearch='';
    ...
    _rA() { if(!this._items.length) return html`<div class="emp">No records</div>`;
    ...
    _rL() { if(!this._items.length) return html`<div class="emp">No records</div>`;
